...
class WhiskyCmd {
    static func install() async {
        let whiskyCmdURL = Bundle.main.url(forResource: "RumCmd", withExtension: nil)

        if let whiskyCmdURL = whiskyCmdURL {
            // swiftlint:disable line_length
            let script = """
            do shell script "ln -fs \(whiskyCmdURL.path(percentEncoded: false)) /usr/local/bin/rum" with administrator privileges
            """
